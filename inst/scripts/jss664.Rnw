...
cbs.segs1 <- GRanges(rep("chr8", nrow(cbs.out)),
		     IRanges(cbs.out$loc.start, cbs.out$loc.end),
		     numberProbes=cbs.out$num.mark,
		     seg.mean=cbs.out$seg.mean, seqlengths=getSequenceLengths(genomeBuild(cnSet))["chr8"])
metadata(cbs.segs1) <- list(genome=genomeBuild(cnSet))
@
...
\subsection*{B allele frequencies,  parallelization, and smoothing for
  large data (many samples/multiple batches)}

One can compute B allele frequencies and log R ratios (LRRs) for each
of the samples and use these as inputs to the HMM.  The B allele
frequencies (BAFs) can be particularly helpful for amplifications. In
the following unevaluated code chunk we create a
\Rclass{BafLrrSetList} object where each element in the list is an
\Rclass{BafLrrSet} for one chromosome. The BAFs and LRRs are
calculated for each batch using the batch-specific genotype cluster
centers.  As the following codechunk takes a while to compute as BAFs
and LRRs are calculated for all chromosomes and all samples, we save
the output of the function to \texttt{outdir} and load from disk.


<<baf>>=
if(!file.exists(file.path(outdir, "brList.rda"))){
	brList <- constructBafLrrSetListFrom(cnSet)
	save(brList, file=file.path(outdir, "brList.rda"))
} else load(file.path(outdir, "brList.rda"))
@
...
An HMM incorporating the B allele frequencies and log R ratios can be
fit using the \Rfunction{hmm} method.  When used with a package
supporting parallel processing, such as the \R{} package
\Rpackage{snow}, the HMM will run over elements (chromosomes) in the
\Robject{oligoSnpSetList} object in parallel.  The HMM will be fit to
all samples in the \Robject{brList} object if the \texttt{sampleIds}
argument is not specified.

<<hmmForOligoSetList,cache=TRUE,eval=FALSE>>=
fit2 <- hmm(brList, sampleIds="NA19007")
fit2 <- fit2[["NA19007"]]
@

Alternatively, if multiple nodes are available one can enable
parallelization.
...
<<hmmForOligoSetListParallel,cache=TRUE>>=
fit2 <- hmm(brList, sampleIds="NA19007")
fit2 <- fit2[["NA19007"]]
@
...
<<latticefig>>=
fitAltered <- fit2[state(fit2) %in% c(1,2,5,6) & chromosome(fit2)=="chr8",]
fig <- xyplotLrrBaf(rd=fitAltered, object=brList[[8]][, "NA19007"],
		    frame=200e3, panel=xypanelBaf,
		    ylim=c(-4, 1.5),
		    cex=0.2,
		    pch=21,
		    col.het="red",
